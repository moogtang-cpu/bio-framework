...
**Results:** Challenging the simplified model that M1 macrophage polarization uniformly impedes wound healing, our analysis reveals that Non-Healing DFUs are characterized by M2 macrophage retention rather than M1 excess, suggesting a failure to complete the normal M1-to-M2 dynamic transition. Healing DFUs exhibited a pronounced M1 bias with robust HIF-1α/NF-κB coupling, while Non-Healing DFUs were locked in a dysregulated M2-dominant state. NF-κB signaling was strongly associated with M1 polarization (Spearman ρ = 0.819), with HIF-1α acting as a synergistic co-activator (ρ = 0.340). The HIF-1α/NF-κB coupling score strongly correlated with M1-M2 polarization balance (ρ = 0.721). A distinct HIF⁺M1 macrophage subcluster (C0) was identified, with 75.9% of cells originating from Healing DFUs and exhibiting markedly elevated hypoxia scores compared to Non-Healing counterparts (P = 1.33 × 10⁻¹⁵⁵). Myeloid cells harbored the highest HIF-1α/NF-κB co-expression rate (34.3% dual-positive). A novel HE-Fibroblast subpopulation (MMP1⁺/MMP3⁺/HIF1A⁺) was 4.2-fold enriched in Healing versus Non-Healing DFUs. Multi-cohort bulk RNA-seq validation confirmed HIF1A upregulation (1.91–4.06-fold), robust HIF1A-NFKB1 correlation (ρ = 0.634–0.708), and consistent upregulation of downstream targets including IL1B (5.56–9.33-fold) and CXCL8 (7.47–23.34-fold). Seven shared HIF-1α/NF-κB target genes (VEGFA, IL6, IL1B, PTGS2, BNIP3, ADM, ANGPTL4) were validated across three or more independent analytical layers.

**Conclusions:** Our findings challenge the conventional understanding of macrophage polarization in DFU pathophysiology, revealing that Non-Healing DFUs are characterized by M2 macrophage retention and impaired HIF-1α/NF-κB coupling-driven M1 activation, rather than by excessive M1 inflammation. This suggests that restoring the capacity for normal M1→M2 dynamic transition, potentially through HIF-1α/NF-κB pathway modulation, represents a promising therapeutic strategy for chronic diabetic wounds.
...
### 3.3 Non-Healing DFUs exhibit M2 macrophage retention and impaired M1 activation

Challenging the conventional view that persistent M1 polarization impedes wound healing, our analysis revealed a striking association between M1 macrophage bias and healing outcome. Scatter plot analysis of M1 versus M2 scores in individual macrophages demonstrated that Healing DFU macrophages were significantly shifted toward an M1-polarized state, whereas Non-Healing DFU macrophages exhibited M2 retention (Figure 4C). This pattern is consistent with a model in which non-healing wounds fail to complete the normal M1→M2 dynamic transition, remaining locked in a dysregulated M2-dominant state rather than suffering from M1 excess.
...
### 4.1 M2 retention and impaired M1 activation in non-healing DFUs: challenging the conventional model
...
This integrated single-cell and bulk transcriptomic study challenges the conventional understanding of macrophage polarization in DFU pathophysiology. Our key findings are:

1. **Non-Healing DFUs are characterized by M2 macrophage retention and impaired M1 activation** — rather than by excessive M1-driven inflammation, as the simplified conventional model assumes. Healing DFUs exhibit robust M1 polarization capacity, suggesting that the ability to mount an adequate initial inflammatory response is associated with wound resolution.
